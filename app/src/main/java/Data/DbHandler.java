...
public class DbHandler extends SQLiteOpenHelper {
    ...
    public doList getList(int id) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            int checkBox = cursor.getInt(3);

            list = new doList(listId, title, desc, checkBox);
        }
    ...
    public List<doList> doAllList(){
        ...
        while(cursor.moveToNext()){
            ...
            int checkBox = cursor.getInt(3);
            // статус
            Log.i("List info", "ID: " + id + " Title: " + title + " Description: " + desciption + "status: " + checkBox +  "\n");

            doLists.add(new doList(id, title, desciption, checkBox)); //
        }
        cursor.close();
        db.close();
